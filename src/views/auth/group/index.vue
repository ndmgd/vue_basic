...
const open = (rowData = {}) => {
  ...
  nextTick(() => {
    ...
    if (rowData) {
      ...
      const checkedKeys = getCheckedKeys(rowData.permissionName)
      treeRef.value?.setCheckedKeys(checkedKeys) // 可选链防实例不存在
      // treeRef.value.setCheckedKeys(rowData.permissionName)
    }
  })
}
